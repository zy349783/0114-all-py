...
for num in range(len(ul)):
    ...
    test = test[["skey", "date", "time", "clockAtArrival", "ordering", "cum_volume", "cum_amount",
                 "open", "close"]]
...
path = np.array(glob.glob('E:\\AMAC\\***'))
path = [os.path.basename(i).split('_')[1].split('.')[0] for i in path]
yester_day = max(path)

dl = [int(yester_day),
      int(datetime.datetime.today().strftime('%Y%m%d'))]
...
try:
    ...
    if data2[data2.duplicated(['date', 'skey'], keep=False)].shape[0] != 0:
        print(data2[data2.duplicated(['date', 'skey'], keep=False)])
    ...
    yesterday = data2['date'].min()
    import zipfile

    f = zipfile.ZipFile('E:\\AMAC\\AMAC_' + str(yesterday) + '.zip', 'r')
    test = pd.read_excel(f.open(f.namelist()[0]))
    test['index_name'] = test['指数名称\nIndex Name'].str[:4] + ' ' + test['指数名称\nIndex Name'].str[4:]
    re2 = test.groupby('index_name')['权重(%)\nWeight(%)'].sum().reset_index()
    re1 = data2[data2['date'] == data2['date'].min()].groupby('index_name')['weight'].sum().reset_index()
    re2['权重(%)\nWeight(%)'] = re2['权重(%)\nWeight(%)'].round(2)
    re1['weight'] = re1['weight'].round(2)
    re = pd.merge(re1, re2, on='index_name', how='outer')
    assert (re[re['weight'] != re['权重(%)\nWeight(%)']].shape[0] == 0)

    write_memb_data(db1, 'index_memb', data2)
    data2.to_csv('E:\\AMAC_weight\\' + str(dl[1]) + '_check.csv')
