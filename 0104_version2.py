...
thisDate = datetime.date(2020, 8, 3)
while thisDate <= datetime.date(2020, 8, 10):
    thisDate_str = str(thisDate).replace('-', '')
    db = DB("192.168.10.178", 'com_md_eq_cn', 'zhenyuy', 'bnONBrzSMGoE')


    mdOrderLog = db.read('md_order', start_date=thisDate_str, end_date=thisDate_str, symbol=[2300824])
    if mdOrderLog is None:
        thisDate = thisDate + datetime.timedelta(days=1)
        continue
    print(thisDate)
    mdTradeLog = db.read('md_trade', start_date=thisDate_str, end_date=thisDate_str, symbol=[2300824])
    ...
    re = mdOrderLog.groupby('skey')['date'].count().reset_index().sort_values(by='date', ascending=False)
    re = re.rename(columns={'date': "count"})
    re = re.sort_values(by='count', ascending=False)
    test_list = re['skey'].values
    test_list1 = test_list[0:20]
    test_list2 = test_list[-1000:]
    try:
    ...
    stockDataLs = [stockData for _, stockData in msgData[msgData['skey'].isin(test_list1)].groupby(['skey'])]
    start = time.time()
    pool = mp.Pool(processes=proc)
    pool.map(mbdGene, stockDataLs)
    pool.close()
    pool.join()
    ...
    for i in range(len(li_st)):
        ...
        if i != len(li_st) - 1:
            ...
            m2 = m
        start = time.time()
        stockDataLs = [stockData for _, stockData in
                       msgData[msgData['skey'].isin(test_list2[m1: m2])].groupby(['skey'])]
