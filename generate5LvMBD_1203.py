...
class generateMBD():
    ...
    def __init__(self, skey, date, hasAuction, prev, ld, yester):

        self.date = date
        self.skey = skey
        self.prev_price = (prev * 10000).round(0)
        self.yes_price = (yester * 10000).round(0)
        self.ld = ld
        ...
        self.auctionBidfrozen = {}
        self.auctionAskfrozen = {}
    ...
    def insertAuctionOrder(self, caa, exchgTm, ApplSeqNum, side, orderType, price, qty):
        if (self.skey > 2300000) & (self.ld <= 5):
            if self.ld == 1:
                m_ax = self.yes_price * 9
            else:
                m_ax = self.prev_price * 9
        else:
            m_ax = 100000000000
        if side == 1:
            if price > m_ax:
                self.auctionBidfrozen[ApplSeqNum] = [price, qty]
            else:
            ...
            if price > m_ax:
                self.auctionAskfrozen[ApplSeqNum] = [price, qty]
            else:
    ...
    def removeOrderByAuctionCancel(self, caa, exchgTm, ApplSeqNum, cancelQty, BidApplSeqNum, OfferApplSeqNum):
        ...
        if cancelApplSeqNum in self.auctionAskfrozen:
            self.auctionAskfrozen.pop(cancelApplSeqNum)
        elif cancelApplSeqNum in self.auctionBidfrozen:
            self.auctionBidfrozen.pop(cancelApplSeqNum)
        else:
    ...
    def insertOrder(self, caa, exchgTm, ApplSeqNum, side, orderType, price, qty):
        ...
        if self.isAuction:
            ...
            if len(self.auctionAskfrozen) != 0:
                for Num in self.auctionAskfrozen:
                    price1 = self.auctionAskfrozen[Num][0]
                    qty1 = self.auctionAskfrozen[Num][1]
                    if price1 in self.askDict:
                        self.askDict[price1] += qty1
                    else:
                        self.askDict[price1] = qty1
                        self.ask_price_levels += 1
                    self.ask_qty += qty1
                    self.ask_amount += qty1 * price1
                    self.ask_order_nums += 1
    ...
    def removeOrderByCancel(self, caa, exchgTm, ApplSeqNum, cancelQty, BidApplSeqNum, OfferApplSeqNum):
        ...
        if cancelApplSeqNum in self.frozenOrderDict:
        ...
        elif cancelApplSeqNum in self.auctionAskfrozen:
            assert(self.auctionAskfrozen[cancelApplSeqNum][1] == cancelQty)
            remain = self.askDict[cancelPrice] - cancelQty
            if remain == 0:
                self.askDict.pop(cancelPrice)
                self.ask_price_levels -= 1
            ...
            print(cancelApplSeqNum)
            print('Bid auction frozen order cancelled here')
        else:
            if cancelSide == 1:
                remain = self.bidDict[cancelPrice] - cancelQty
                if remain == 0:
