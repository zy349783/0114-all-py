        ...
        try:
            for rowEntry in stockMsgNP:
                if rowEntry.isAuction:
                    ...
                    if rowEntry.ApplSeqNum >= 24935294:
                        if rowEntry.status == 'order':
                        ...
                        if rowEntry.status == 'order':
                            ...
                            simMarket.removeOrderByTrade(rowEntry.clockAtArrival, rowEntry.TransactTime,
                                                         ...
                                                         rowEntry.BidApplSeqNum,
                                                         rowEntry.OfferApplSeqNum)
                        if (21534857 in simMarket.asknq[8.25]) & (rowEntry.ApplSeqNum <= 21566010):
                            print(str(rowEntry.ApplSeqNum) + ' ' + str(simMarket.asknq[8.25][21534857]) + ' ' + str(simMarket.asknq[8.25].keys()))
            ...
            readPath = '/mnt/e/filter/' + str(data['skey'].iloc[0]) + '.pkl'
            sizeData = pd.read_pickle(readPath)
            sizeData['amountFilter'] = sizeData['amountFilter'].fillna(0)
            sizeFilter = sizeData[sizeData['date'] == data['date'].iloc[0]]['amountFilter'].values[0]
            ...
            data = data[['skey', 'date', 'time', 'clockAtArrival', 'datetime', 'ordering', 'ApplSeqNum', 'bbo_improve',
                         ...
                         'bid10qList', 'bid9qList', 'bid8qList', 'bid7qList', 'bid6qList', 'bid5qList', 'bid4qList',
                         'bid3qList', 'bid2qList', 'bid1qList',
                         'ask1qList', 'ask2qList', 'ask3qList', 'ask4qList', 'ask5qList', 'ask6qList', 'ask7qList',
                         'ask8qList', 'ask9qList', 'ask10qList',
                         ...
                         'total_bid_orders', 'total_ask_orders', 'total_bid_levels', 'total_ask_levels',
...
class SimMktSnapshotAllNew():
    ...
    def insertOrder(self, clockAtArrival, exchangeTime, seqNum, side, orderType, price, qty, applySeqNum):
        if self.isAuction:
            ...
            self.sequenceNum = []
            self.bboImprove = []
            self.updateMktInfo(auctionClockAtArrival, auctionExchangeTime, auctionSeqNum, auctionBBOImprove,
                               record=True)
    ...
    def removeOrderByCancel(self, clockAtArrival, exchangeTime, seqNum,
        ...
        if cancelApplSeqNum in self.tempOrder:
            ...
            if tempOrderType == '2':
                if cancelApplSeqNum in self.asknq[cancelPrice]:
                    self.asknq[cancelPrice][cancelApplSeqNum] = np.int32(
                    ...
                    self.bidnq[cancelPrice][cancelApplSeqNum] = np.int32(
    ...
    def getAllInfo(self):
        ...
        targetCols = (['time', 'clockAtArrival', 'sequenceNo', 'StockID', 'cum_volume', 'cum_amount', 'close', 'open',
                       'bbo_improve'] +
...
if __name__ == '__main__':
    ...
    while thisDate <= datetime.date(2020, 2, 21):
        ...
        mdOrderLog = db.read('md_order', start_date=thisDate_str, end_date=thisDate_str, symbol=2000009)
        if mdOrderLog is None:
            thisDate = thisDate + datetime.timedelta(days=1)
            continue

        print(thisDate)
        mdTradeLog = db.read('md_trade', start_date=thisDate_str, end_date=thisDate_str, symbol=2000009)
        mdLog = db.read('md_snapshot_l2', start_date=thisDate_str, end_date=thisDate_str, symbol=2000009)
        orders_data = {}
        trades_data = {}
        prev_data = {}

        if 'pandas' in str(type(mdOrderLog)):
            orders_data[2000009] = mdOrderLog
        if 'pandas' in str(type(mdTradeLog)):
            trades_data[2000009] = mdTradeLog
        if 'pandas' in str(type(mdTradeLog)):
            prev_data[2000009] = mdLog['prev_close'].iloc[0]
