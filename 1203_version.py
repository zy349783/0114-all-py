...
def mbdGene(stockData):
    ...
    try:
        ...
        prev = read_stock_daily(db1, 'mdbar1d_tr', start_date=int(thisDateStr), end_date=int(thisDateStr), skey=[int(thisStock)])['closeL1'].iloc[0]
        ld = read_stock_daily(db1, 'mdbar1d_tr', start_date=int(thisDateStr), end_date=int(thisDateStr), skey=[int(thisStock)])['listed_days'].iloc[0]
        yester = read_stock_daily(db1, 'mdbar1d_tr', start_date=int(thisDateStr), end_date=int(thisDateStr), skey=[int(thisStock)])['yclose'].iloc[0]
        simMarket = generateMBD(skey=thisStock, date=int(thisDateStr), hasAuction=hasAuction, prev=prev, ld=ld, yester=yester)
        ...
        for rowEntry in stockDataNP:
            ...
            if rowEntry.isAuction:
                ...
                print(rowEntry)
                if 920000 in simMarket.askDict:
                    print(simMarket.askDict[920000])
                    print(simMarket.asknq1[920000])
                if rowEntry.ApplSeqNum == 21145147:
                    print(rowEntry)
                # print(simMarket.bid1p)
                # print(simMarket.ask1p)
                # if rowEntry.ApplSeqNum >= 276155:
                #     if 276155 in simMarket.frozenOrderDict:
                #         print(simMarket.frozenOrderDict[276155])
                # if rowEntry.ApplSeqNum == 282795:
                #     print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
                if rowEntry.status == 'order':
        ...
        try:
            ...
            print('We cant find size filter data')
...
thisDate = datetime.date(2020, 8, 24)
while thisDate <= datetime.date(2020, 8, 24):
    thisDate_str = str(thisDate).replace('-', '')
    db = DB("192.168.10.178", 'com_md_eq_cn', 'zhenyuy', 'bnONBrzSMGoE')


    mdOrderLog = db.read('md_order', start_date=thisDate_str, end_date=thisDate_str, symbol=[2300869])
    if mdOrderLog is None:
        thisDate = thisDate + datetime.timedelta(days=1)
        continue
    print(thisDate)
    mdTradeLog = db.read('md_trade', start_date=thisDate_str, end_date=thisDate_str, symbol=[2300869])
