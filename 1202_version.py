...
class go():
    ...
    def run(self, s):
        ...
        try:
            ...
            data = data[
                ['skey', 'date', 'time', 'clockAtArrival', 'ordering', 'ApplSeqNum', 'bbo_improve', 'pass_filter',
                 'cum_trades_cnt', 'cum_volume', 'cum_amount',
                 'prev_close', 'open', 'close', 'bid10p', 'bid9p', 'bid8p', 'bid7p', 'bid6p', 'bid5p', 'bid4p', 'bid3p',
                 'bid2p', 'bid1p',
...
class SimMktSnapshotAllNew():
    ...
    def getAllInfo(self):
        ...
        targetCols = (['time', 'clockAtArrival', 'sequenceNo', 'StockID', 'cum_volume', 'cum_amount', 'cum_trades_cnt',
                       'close', 'bbo_improve'] +
...
#     # start date
#     thisDate = datetime.date(2020, 1, 4)
#     while thisDate <= datetime.date(2020, 7, 31):
...
#         sl = read_memb_daily(db1, 'index_memb', index_id=[1000852], start_date=20170901, end_date=20201203)[
#             'skey'].unique()
#         sl = sl[sl > 2000000]
#         assert (len(sl) == 1028)
#         sl = list(set(sl1) - set(sl))
#         os.mkdir('/mnt/e/mbd_data/' + thisDate_str)
#
...
#             pool.map(g.run, test_list[m1: m2])
#             pool.close()
#             pool.join()
...
if __name__ == '__main__':
    ...
    db = DB("192.168.10.178", 'com_md_eq_cn', 'zhenyuy', 'bnONBrzSMGoE')
    # start date
    thisDate = datetime.date(2020, 11, 12)
    while thisDate <= datetime.date(2020, 11, 12):
        intDate = (thisDate - datetime.date(1899, 12, 30)).days
        thisDate_str = str(thisDate).replace('-', '')
        sl = [2300884]
